 ...
 */
Eigen::VectorXf PD_Ui_faster(Eigen::VectorXf & z_i, Eigen::VectorXf & x, Eigen::VectorXf & p,
		float rho, int I0, int n)
{
	return (rho*z_i - rho*x - p);
}
/*
 * function to calculate the partial derivative of Ui(zi) in GPU: slide 20 (eq. 17)
 */
viennacl::vector<float> PD_Ui(viennacl::vector<float>& z_i, viennacl::vector<float>& x,
		viennacl::vector<float>& p, float rho, int I0, int n)
{
	viennacl::vector<float> tmp1 = rho*z_i;
	viennacl::vector<float> tmp2 = rho*x;
	return (tmp1 - tmp2 - p);
}
 ...
 */
viennacl::vector<float> PD_Gi_faster(viennacl::vector<float>& z_i_gpu,
 ...
 */
viennacl::vector<float> PD_Hi_faster(viennacl::vector<float>& z_i_gpu,
						viennacl::compressed_matrix<float>& BT_i_gpu,
						viennacl::vector<float>& neg_exp_z_i_gpu,
						viennacl::vector<float>& Mi_gpu,
						int I0){
...
//	viennacl::vector<float> x_i_gpu(n);
//	copy(X_i, x_i_gpu);
//
//	viennacl::vector<float> p_i_gpu(n);
//	copy(P_i, p_i_gpu);
	...
	copy(neg_exp_z_i_gpu,neg_exp_z_i); // fixme: needed?
	...
	while(error > 1e-10)
	{
		...
		pdgi = PD_Gi_faster(z_i_gpu, AT_i_gpu, BT_i_gpu, neg_exp_z_i_gpu, numerator_gpu,Mi_gpu, I0, n, m);
		pdhi = PD_Hi_faster(z_i_gpu, BT_i_gpu, neg_exp_z_i_gpu, Mi_gpu, I0);
		sum_pd = pdgi +pdhi;
		copy(sum_pd, sum_pd_cpu);
		pdui = PD_Ui_faster(z_i, X_i, P_i, rho, I0, n);
		sum_pd_cpu += pdui;
...
int main(int argc, char** argv) {
	...
	// TODO: 1- tune parameters and initializations, 2- calculate PSNR between output image and ground truth
	// CURRENT STATUS: HADAMARD_MATRIX_VECTOR_TIMED IS A BIT FASTER.
	// IN THE KERNEL, COPY THE OUTPUT VECTOR TO A SPARSEMATRIX I A LOOP, CHECK IF THIS IS FASTER!
	// IF WE CAN INITIALIZE A SPARSE MATRIX FROM THE KERNEL OUTPUT DIRECTLY, IT WILL BE EVEN FASTER
	// OTHERWISE CHECK IF USING KERNEL WITH IMAGE OBJECTS CAN HELP MAKE IT FASTER
	...
	int useHadamardKernel = 0;   // Hadamard matrix vector product: 1->use custom kernel, 0->use Eigen+viennacl function (slower)
	...
	int I_0; //6e4 250
	float alpha; // alpha in matrix gamma, can be tuned
	float beta; // needs to be tuned
	float sigma; // sigma can be 100, 150, 200,...500
	float rho;   // 0.1 -> 10
	int n_iterations;  // ADMM number of iterations

	std::cout<<"Enter int I0 in the range of 200 to 500 "<<std::endl;
	std::cin >> I_0;
	std::cout<<"Enter float alpha in the range of 0.1 to 10"<< std::endl;
	std::cin >> alpha;
	std::cout<<"Enter float beta in the range of 0.1 to 10"<< std::endl;
	std::cin >> beta;
	std::cout<<"Enter float sigma in the range of 100 to 500"<<std::endl;
	std::cin >> sigma;
	std::cout<<"Enter float rho in the range of 200 to 500"<<std::endl;
	std::cin >> rho;
	std::cout<<"Enter int number of iterations in the range of 20 to 200"<<std::endl;
	...
	{
		Src[i] = cv::imread(
				"../Images/PCB/LR" + boost::lexical_cast<std::string>(i + 1)
						+ ".tif", CV_LOAD_IMAGE_GRAYSCALE   ); //CV_LOAD_IMAGE_ANYDEPTH fixme

		Src[i].convertTo(Src[i], CV_32FC1);

		//TODO: should be out of the loop
		dest = cv::Mat(Src[0].rows * rfactor, Src[0].cols * rfactor, CV_32FC1 ); //CV_16UC1
	...
	gnd_truth = cv::imread(
					"../Images/PCB/origin.tif", CV_LOAD_IMAGE_GRAYSCALE);//CV_LOAD_IMAGE_GRAYSCALE
	...
	//std::cout << "Write bicubic Image flag= " << write_bool << std::endl;
	//std::cout << "---------------------------------------PSNR(bicubic & ground truth):" << getPSNR(f_y2_mat, gnd_truth) << std::endl;
	...
	if(iterate_flag==1)
	{
		...
		for(int iter = 0; iter<n_iterations; iter++)
		{
			...
			for (int i = 0;  i < i_uBound; i++) {
				if(i < M) // ADAM Step Update
				{
					ADAM(z_k[i] , z_k_viennacl[i],
							...
							BT_i[i], DHFT2[i],
							Y[i], y_viennacl[i],
							sigmasqEig, T[i], x_k, p_k[i], rho, rhoEig, I_0, n, m);
			...
			viennacl::vector<float> x_k_vienna(n), p_k_vienna(n), rho_vec(n);
